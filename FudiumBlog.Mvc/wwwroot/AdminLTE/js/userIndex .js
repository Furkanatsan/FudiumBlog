$(document).ready(function () {
   const dataTable= $('#usersTable').DataTable({
    ...
    $(function () {
        ...
        $('#btnAdd').click(function () {
            $.get(url).done(function (data) {
                ...
                placeHolderDiv.find(".modal").modal('show');//ekrana gelmesini sağlar.
            });
        });
        //Ajax get
        //Ajax post kullanıcı ekleme kaydetme(form datayı userAddDto olarak gönderme)
        placeHolderDiv.on('click', '#btnSave', function (event) {
            event.preventDefault();
            const form = $('#form-user-add');//form seçildi
            const actionUrl = form.attr('action');//actiona ulaşmak için
            const dataToSend = new FormData(form.get(0))//0. indexteki formun verilerini aldık
            $.ajax({
                url: actionUrl,
                type: 'POST',
                data: dataToSend,
                ...
                success: function (data) {
                    ...
                    if (isValid) {
                        placeHolderDiv.find('.modal').modal('hide');//ekrandan gitmesini sağlar
                        const newTableRow=dataTable.row.add([
                        ...
                        toastr.success(`${categoryAddAjaxModel.UserDto.Message}`, 'Başarılı İşlem!');
                    }
                    else {
                        let summaryText = "";
    ...
    $(document).on('click', '.btn-delete', function (event) {
        ...
        const userName = tableRow.find('td:eq(1)').text();//2.sıradaki td nin ismini çektik.
        Swal.fire({
            title: 'Emin Misiniz?',
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (userDto.ResultStatus === 0) {
                            Swal.fire(
                                'Silindi!',
                                `${userDto.User.UserName} adlı kullanıcı silinmiştir.`,
                                'success'
                            );
                            dataTable.row(tableRow).remove().draw();//datatable api
                        }
                        else {
                            Swal.fire({
                                icon: 'error',
     ...
     //Ajax GET//Güncelleme(_UserUpdatePartial)
    $(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (userUpdateAjaxModel.UserDto!=null) {
                        ...
                        const tableRow = $(`[name="${id}"]`);//bu id ye sahip name attr olan tableRowu seçtik
                    }
                    ...
                    if (isValid) {
                        ...
                        tableRow.attr("name", `${id}`);//datatablenin name özelliğini
                        dataTable.row(tableRow).invalidate();//eski tablerowun değiştiğini belirtiyoruz.
